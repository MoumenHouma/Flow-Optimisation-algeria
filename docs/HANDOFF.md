...
pick up the project with full context. Snapshot as of **2026-07-22**.
...
Migrations present: **0001 → 0012**.

**Last green run:** backend 76 pytest (77% cov, CI gate 70) · worker 17 pytest
(84% cov, CI gate 75) · frontend 49 vitest + build. `ruff`/`black`/`mypy`/`eslint`/`tsc` clean.
...
## 8. Next: Phase 4 (not started)

From `docs/PRD.md`:
- **Payments** — CCP / BaridiMob integration (COD is 95% of the market; e-payment is now mandated by the 2025–2026 cahier des charges).
- **Driver-PWA offline queue** — background-sync mutation queue for status updates / proof capture without connectivity.
- **Turn-by-turn navigation** in the driver app.
- (Stretch) worker-side sklearn regressor upgrade for F13 service-time prediction.

---

## Suggested first prompt to resume

> "Read CLAUDE.md and docs/PRD.md. We've finished F1–F16 + hardening (see
> docs/HANDOFF.md). I want to start Phase 4 — begin with [payments / offline
> queue / turn-by-turn]. Propose a plan first."
